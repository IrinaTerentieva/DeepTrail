...
# ---------------------------
# cd /home/irina/HumanFootprint/Scripts/training
# python SegFormer_HumanFootprint_Metaflow.py run
# ---------------------------

class TrailSegmentationFlow(FlowSpec):
    ...
    def start(self):
        """
        Load the configuration file and initialize settings.
        """
        ...
        # Load experiment settings from YAML
        self.learning_rate = self.config['experiments']['learning_rates'][0]
        self.dataset_name = list(self.config['experiments']['datasets'].keys())[0]
        self.dataset_path = self.config['experiments']['datasets'][self.dataset_name]
        self.architecture = self.config['experiments']['architectures'][0]
        self.model_config = self.config['models'][self.architecture]
        ...
        run_name = (
            ...
            f"dataset_{self.dataset_name}_arch_{self.architecture}_lr_{self.learning_rate}_batch_{self.batch_size}"
        )
