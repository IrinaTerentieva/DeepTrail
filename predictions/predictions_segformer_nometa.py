...
def sliding_window_prediction(image_path, model, output_dir, patch_size, stride):
    ...
    # Threshold at 0.1, remove small connected components (< 200 pixels)
    threshold = 0.1
    ...
    output_ds = ds.isel(band=0).copy(deep=True)  # Copy metadata from first band

    # Remove metadata issue
    if "long_name" in output_ds.attrs:
        del output_ds.attrs["long_name"]

    output_ds.values = scaled_prediction  # Assign prediction data
...
def run_prediction():
    ...
    # List image files
    image_files = [os.path.join(test_image_path, f) for f in os.listdir(test_image_path) if f.endswith('.tif')] if os.path.isdir(test_image_path) else [test_image_path]
