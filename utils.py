...
def sliding_window_prediction_tf(image_path, model, patch_size, stride, output_path, reference_image_path, scale_factor=100, inverted = False):
    """
...
def sliding_window_prediction_torch(image_path, model, patch_size, stride, output_path, reference_image_path, scale_factor=100, inverted=False, device="cuda"):
    """
    Perform sliding window prediction using PyTorch and save each patch prediction incrementally.
            ...
            with torch.no_grad():  # Disable gradient calculations for inference
                for i in tqdm(range(0, height - patch_size + 1, stride), desc="Sliding Window Rows"):
                    for j in tqdm(range(0, width - patch_size + 1, stride), desc="Sliding Window Cols", leave=False):
                        ...
                        # Normalize patch
                        patch = normalize_image(patch)
                        patch = torch.tensor(patch, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(device)  # Add batch and channel dims
