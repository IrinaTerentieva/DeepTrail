...
def save_image_mask_pair(image, mask, idx, figures_dir):
    """
    Save a figure with an image and its corresponding mask side by side.
    If the image is in CHW format (e.g. (3, H, W)), it is transposed to HWC.
    """
    # If image is in CHW format (and has 3 or 4 channels), convert to HWC
    if image.ndim == 3 and image.shape[0] in [3, 4]:
        image = np.transpose(image, (1, 2, 0))

    fig, axs = plt.subplots(1, 2, figsize=(12, 6))
    # Display the image without a colormap if it's RGB
    axs[0].imshow(image)
    ...
    plt.tight_layout()
    out_path = os.path.join(figures_dir, f"sample_{idx}.png")
...
class WetTrailsDataset(Dataset):
    ...
    Args:
        ...
        transform (albumentations.Compose, optional): Augmentation pipeline.
            To disable augmentation, pass A.Compose([]).
    """
    ...
    def __init__(self, data_dir, threshold=0, min_area=100, skip_threshold=0.01, transform=None):
        ...
        # To disable augmentation, pass an empty Compose: A.Compose([])
        ...
        # Collect valid image files with a corresponding label file.
        all_image_files = [f for f in os.listdir(data_dir) if f.endswith('_image.tif')]
        # Uncomment to debug all files found:
        # print("All image files found:", all_image_files)
        self.valid_files = []
        ...
        # Uncomment to check valid files:
        # print(f"Valid files ({len(self.valid_files)}):", self.valid_files)
    ...
    def __getitem__(self, idx):
        ...
        with rasterio.open(image_path) as img_src:
            image = img_src.read().astype(np.float32)[:3, :, :]  # use only first 3 channels
            if image.shape[0] > 1:
        ...
        # Debug: print if any NaNs/Infs appear in the image.
        if np.isnan(image).sum()>0:
            print('************************ Panic at the disco poof poof!')
        ...
        if isinstance(image, torch.Tensor):
            ...
            im_np = image
        # print("Image stats: min =", np.min(im_np), ", max =", np.max(im_np), ", mean =", np.mean(im_np))
        ...
        if isinstance(mask, torch.Tensor):
            ...
            mask_np = mask
        # print("Label unique values:", np.unique(mask_np))
