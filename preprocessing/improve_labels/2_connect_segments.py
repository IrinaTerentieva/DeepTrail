...
'''
Script for connecting trail and seismic line segments based on geospatial data.

Purpose:
This script processes centerline files (GeoPackages) and corresponding raster files to:
1. Merge connected line segments into unified geometries.
2. Classify lines based on their lengths as 'short' or 'long' trails.
3. Connect line endpoints using a cost-based approach derived from probability maps.
4. Filter out short lines and save connected outputs for each input file.

Key Features:
- Parallel processing of files in a specified directory.
- Cost-based routing for connecting segments using raster data.
- Classification of trails by length.
- Output saved as GeoPackage files.

Inputs:
- Centerline GeoPackages with line geometries.
- Corresponding rasters for cost-based routing.

Outputs:
- Connected line segments saved in the specified output folder.

Parameters:
- `centerline_folder`: Folder containing centerline GeoPackages.
- `raster_folder`: Folder containing corresponding raster files.
- `output_folder`: Directory for saving connected outputs.
- `threshold_distance`: Maximum distance for connecting segments.
...
def connect_segments(
    ...
    raster_dir = os.path.dirname(raster_path)  # Get the directory of the raster file
    points_folder = os.path.join(raster_dir, "points_output")  # Add 'points_output' to the directory path
    os.makedirs(points_folder, exist_ok=True)
    points_output_path = os.path.join(raster_dir, points_folder, f"{os.path.basename(raster_path)}_points.gpkg")
    ...
    def process_trails(indices, check_points, max_connections, points_output_path, prioritize_long_trail=True):
        for i in indices:
    ...
    process_trails(long_trail_indices, check_points=long_trail_check_points, max_connections=long_trail_max_connections, points_output_path = points_output_path)

    print('Process short trails')
    # Connect the remaining trails
    process_trails(range(len(all_coords)), check_points=short_trail_check_points, max_connections=short_trail_max_connections, points_output_path = points_output_path)
    ...
    print('Saving: ', points_output_path)
...
def main():
    ...
    centerline_folder = '/home/irina/HumanFootprint/DATA/TrainingCNN/UNet_patches1024_nDTM10cm/centerline'
    raster_folder = '/home/irina/HumanFootprint/DATA/TrainingCNN/UNet_patches1024_nDTM10cm/'
