...
def merge_connected_lines(gdf):
    """
    Merge all intersecting or connected lines.
    """
    merged_lines = unary_union(gdf.geometry)
    ...
    if merged_lines.geom_type == 'LineString':
        ...
        return gpd.GeoDataFrame({'geometry': list(merged_lines.geoms), 'length': [line.length for line in merged_lines.geoms]}, crs=gdf.crs)
    else:
        raise ValueError(f"Unexpected geometry type: {merged_lines.geom_type}")


def classify_trail_length(length, length_median=10):
    """Classify trail length into 'short' or 'long'."""
    return 'long_trail' if length >= length_median else 'short_trail'


def connect_segments(
    gdf, raster_path, connection_threshold=200, high_cost_factor=1.5,
    low_prob_threshold=3, long_trail_check_points=20, long_trail_max_connections=5,
    short_trail_check_points=5, short_trail_max_connections=1
):
    """
    Connects broken trail segments based on raster cost maps.
    """
    gdf = merge_connected_lines(gdf)
    ...
    gdf['classification'] = gdf['length'].apply(lambda x: classify_trail_length(x, threshold_length))
    ...
    for idx, row in gdf.iterrows():
        ...
        if not line.is_empty and line.geom_type == 'LineString':
            all_points.append(Point(line.coords[0]))  # Start
            all_points.append(Point(line.coords[-1]))  # End
            line_ids.extend([idx, idx])
    ...
    def point_to_raster_indices(point):
        """Convert spatial coordinates to raster indices."""
        col, row = ~transform * (point.x, point.y)
        return int(row), int(col)

    cost_map = np.array(1 - probability_map, dtype=np.float32)
    cost_map = np.where(cost_map > 0.5, cost_map * high_cost_factor, cost_map)
    cost_map[probability_map < low_prob_threshold] = 100  # Penalize low probabilities

    connections = []
    connection_data = []

    def process_trails(indices, check_points, max_connections, prioritize_long=True):
        for i in indices:
            distances, nearest_indices = tree.query(all_coords[i], k=min(len(all_coords), check_points + 1))
            nearest_indices = nearest_indices[1:]  # Skip itself

            if prioritize_long and gdf.loc[line_ids[i], 'classification'] == 'long_trail':
                nearest_indices = [idx for idx in nearest_indices if gdf.loc[line_ids[idx], 'classification'] == 'long_trail'] or nearest_indices
            ...
            for nearest_idx in nearest_indices:
                ...
                try:
                    start_idx = point_to_raster_indices(all_points[i])
                    end_idx = point_to_raster_indices(all_points[nearest_idx])

                    if not (0 <= start_idx[0] < cost_map.shape[0] and 0 <= start_idx[1] < cost_map.shape[1]):
                        continue
                    if not (0 <= end_idx[0] < cost_map.shape[0] and 0 <= end_idx[1] < cost_map.shape[1]):
                        continue

                    path_indices, _ = route_through_array(cost_map, start_idx, end_idx, fully_connected=True)
                    path_coords = [transform * (col, row) for row, col in path_indices]

                    if len(path_coords) > 1:
                        path_line = LineString(path_coords)
                        connections.append(path_line)

                        classification_1 = gdf.loc[line_ids[i], 'classification']
                        classification_2 = gdf.loc[line_ids[nearest_idx], 'classification']
                        conn_type = 'long-long' if classification_1 == 'long_trail' and classification_2 == 'long_trail' else 'short-short' if classification_1 == 'short_trail' and classification_2 == 'short_trail' else 'short-long'

                        connection_data.append((path_line, conn_type))
                        connections_made += 1
    ...
    connections_gdf = gpd.GeoDataFrame({'geometry': [conn[0] for conn in connection_data], 'connection_type': [conn[1] for conn in connection_data]}, crs=gdf.crs)
...
def process_file(centerline_path, raster_path, output_folder, threshold_distance):
    """
    Process a single matched centerline-raster pair.
    """
    base_name = os.path.splitext(os.path.basename(centerline_path))[0]
    print(f"Processing: {centerline_path} with {raster_path}")

    # Load centerline GeoPackage
    gdf = gpd.read_file(centerline_path)

    # Call the connection function
    simplified_gdf = connect_segments(gdf, raster_path, connection_threshold=threshold_distance)
...
def process_files_in_folder(centerline_folder, raster_folder, output_folder, threshold_distance=200, max_workers=4):
    """
    Match centerline (.gpkg) and raster (.tif) files based on names and process them in parallel.
    """
    os.makedirs(output_folder, exist_ok=True)

    # Get normalized filenames for better matching
    centerline_files = {
    ...
    # Find matching pairs (ignoring suffix differences)
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_file, center, raster, output_folder, threshold_distance): (center, raster) for center, raster in matching_files}
...
if __name__ == "__main__":
    ...
    raster_folder = "/media/irina/My Book1/Conoco/DATA/Drone_PPC_2024/ndtm"
    ...
    process_files_in_folder(centerline_folder, raster_folder, output_folder, threshold_distance=200, max_workers=8)
